...
class Scanner:
    ...
    def _analyse(self, sym: dict) -> Optional[dict]:
        ...
        # ── STRATEGY FILTERS (backtest-derived, all must pass) ────────
        strategy_block = self._apply_strategy_filters(
            symbol, quality_score, additional,
        )
        ...
        premium = (additional.get("vol_24h_usdt") or 0) >= 20_000_000
    ...
    def _apply_strategy_filters(
        self, symbol: str, quality_score: int, add: dict,
    ) -> Optional[str]:
        """
        Backtest-derived strategy filters — ALL must pass.
        Called after quality score is known, before cooldown is recorded.

        1. BTC trend must be 'ranging'  (pumping/dumping/unknown → skip)
        2. Quality score 3–6            (QS 7-8 underperforms; QS <3 too weak)
        3. Market cap < $200M           (skip if data available and ≥ $200M)
        4. funding_rate >= 0 AND funding_in_ideal_range == True
        5. OI change 5% – 50%           (new money, no overextension)
        6. 24h volume ≥ $5M             (basic liquidity)
        """
        # 1. BTC trend must be ranging
        if self._btc_trend != "ranging":
            logger.info(
                "%s  STRATEGY rejected — BTC trend is '%s' (must be ranging)",
                symbol, self._btc_trend,
            )
            return "btc_not_ranging"

        # 2. Quality score 3–6
        if not (3 <= quality_score <= 6):
            logger.info(
                "%s  STRATEGY rejected — quality_score %d not in 3–6",
                symbol, quality_score,
            )
            return "quality_score_range"

        # 3. Market cap < $200M (allow None = unknown/very small coin)
        mcap = add.get("market_cap_usd")
        if mcap is not None and mcap >= 200_000_000:
            logger.info(
                "%s  STRATEGY rejected — market_cap %s ≥ $200M",
                symbol, self._fmt_vol_usd(mcap),
            )
        ...
        funding = add.get("funding_rate")
        if funding is None:
            logger.info("%s  STRATEGY rejected — funding_rate unavailable", symbol)
            return "funding_unavailable"
        funding_ideal = add.get("funding_in_ideal_range", False)
        if not (funding >= 0 and funding_ideal):
            logger.info(
                "%s  STRATEGY rejected — funding_rate %.4f%% failed (>=0: %s, ideal_range: %s)",
                symbol, funding, funding >= 0, funding_ideal,
            )
            return "funding_out_of_range"

        # 5. OI change 5% to 25%  [TIGHTENED from 50% — high OI = liquidation risk]
        oi_chg = add.get("oi_change_pct")
        if oi_chg is None:
            logger.info("%s  STRATEGY rejected — oi_change_pct unavailable", symbol)
            return "oi_unavailable"
        if not (5.0 <= oi_chg <= 25.0):
            logger.info(
                "%s  STRATEGY rejected — oi_change_pct %.2f%% not in [5%%, 25%%]",
                symbol, oi_chg,
            )
            return "oi_out_of_range"

        # 6. 24h volume ≥ $5M
        vol_24h = add.get("vol_24h_usdt")
        if vol_24h is None or vol_24h < 5_000_000:
            logger.info(
                "%s  STRATEGY rejected — vol_24h_usdt %s < $5M",
                symbol, self._fmt_vol_usd(vol_24h or 0),
            )
            return "vol_24h_too_low"

        logger.info(
            "%s  ✅ Strategy filters passed  (QS:%d  mcap:%s  funding:%.4f%%  OI:%.1f%%  vol:%s)",
            symbol, quality_score,
            self._fmt_vol_usd(mcap) if mcap else "unknown",
            funding, oi_chg, self._fmt_vol_usd(vol_24h),
        )
        return None
    ...
    def _send_startup(self) -> None:
        lines = [
            ...
            "",
            "<b>Strategy Filters (backtest-derived, all must pass):</b>",
            "📊 BTC trend = ranging only",
            "⭐ Quality score 3–6 (not 0-2 or 7-8)",
            "💰 Market cap &lt; $200M",
            "💸 Funding rate 0% to 0.15%",
            "📈 OI change 5% to 50%",
            "💧 24h volume ≥ $5M",
            "",
