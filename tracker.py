...
class SignalTracker:
    ...
    def store_tp_decision(self, symbol: str, signal_time: str, tp: int, decision: dict) -> bool:
        """
        Thread-safe write of decision data into signal[f'tp{tp}_snapshot']['decision'].
        Called by StrategyManager after processing a TP hit.
        Returns True if the signal was found and updated.
        """
        with self._lock:
            signals = self._load(self._signals_file)
            for sig in signals:
                if sig.get("symbol") == symbol and sig.get("alert_time") == signal_time:
                    snap_key = f"tp{tp}_snapshot"
                    snap = sig.get(snap_key) or {}
