...
- Paper trading mode: simulate full trade lifecycle (entry, TPs, SL, time limits) with zero real orders — tracks fake P&L in data/paper_account.json, sends 📝-prefixed Telegram alerts for every event
- Live trading + paper trading: both use same trades.json (paper flag); StrategyManager handles all paper trade lifecycle (SL detection, TP exits, trailing SL, time limits, emergency closes)
- Rich TP decision alerts for every TP hit (paper and live): score/10 with per-component breakdown, action chosen + reason, fill details, SL movement reason, time limit window, paper balance
- Decision data stored at trade["tp_decisions"]["tp{N}"] in trades.json and in signal["tp{N}_snapshot"]["decision"] in signals.json for post-analysis
...
| `trader.py` | Trade entry — paper and live; manages paper_account.json |
| `strategy_manager.py` | Full TP/SL exit strategy — paper and live; rich decision alerts |
| `config.json` | Configuration file (thresholds, scan settings) |

## Paper Trading

Set in `config.json` under `"trading"`:
...
With `paper_mode: true` and `trading.enabled: true`:
- All trade entries are simulated — no real orders placed
- Real-time Binance price data is still used for fills and SL/TP checks
- Paper account tracked in `data/paper_account.json`
- Every TP hit, SL hit, time-limit exit, and daily summary sent to Telegram with 📝 prefix
- `exit_strategy.enabled` is required to simulate the TP-ladder logic
